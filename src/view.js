...
const renderViewedPosts = (elements, state) => {
  const { ui: { viewedPosts: list } } = state;
  const { posts: postsWrapper } = elements;
  const renderedPosts = postsWrapper.getElementsByTagName('LI');
  const viewedPosts = Array.from(renderedPosts).filter((post) => {
    const currentId = Number(post.querySelector('A').dataset.postId);
...
const renderPosts = (elements, state, i18n) => {
  ...
  sortedPostsData.forEach(({
    ...
    a.dataset.markAsVisited = true;
    a.textContent = title;

    const button = document.createElement('BUTTON');
    button.dataset.postId = id;
    button.dataset.markAsVisited = true;
    button.dataset.action = 'openModal';
